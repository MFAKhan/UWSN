...
public class GA4 {
	...
	public GA4_Individual Best;
	public double fitness;

	public GA4(final SimulationMap Map, final int NumNodes, final int NumAUVs, final double Speed,
			final String DistanceType, final double DistanceScale, final double TimeStamp, final ArrayList<Integer> T) {
		GA4_Population myPop = new GA4_Population(100, true, NumNodes, T);
		int generationCount = 0;
		while (generationCount < 100) {
			generationCount++;
			myPop = GA4_Algorithm.evolvePopulation(myPop, Map, NumNodes, NumAUVs, Speed, DistanceType, DistanceScale,
					TimeStamp, T);
		}
