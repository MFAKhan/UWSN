...
public class ResurfacingSimulation {
	...
	public final static int Y_DIM = 128; // No. of Nodes Vertically in Mesh
	...
	public ResurfacingSimulation() {
	}

	public double Run(final int NumResurfaceStops) {
		for (int i = 0; i < SAMPLES_PER_TOUR; i++) {
			this.SingleSimulationRun(NumResurfaceStops);
		}
		this.AnalyzeExperiment();
		// this.PrintAverages(NumResurfaceStops);
		return this.results[0][0];
	}

	public void SingleSimulationRun(final int NumResurfaceStops) {
		 ...
		 * System.out.println();
		 */
		this.CreateAndEvaluateTour(MyMap, LastPacketTS, NumResurfaceStops);
	...
	void PrintAverages(final int ResurfaceStops) {
